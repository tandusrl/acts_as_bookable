module ActsAsBookable::Bookable
  module Core
    ...
    module ClassMethods
      ...
      #
      # @raise ActsAsBookable::InvalidOptions if options are not valid
      #
      def validate_booking_options!(options)
        ...
        wrong_types = required_params
          .select{ |k,v| options.has_key?(k) && options[k].class.to_s != v }
          .map{ |k,v| "'#{k}' must be a '#{v}'" }
        required_params = required_params
          .select{ |k,v| !options.has_key?(k) }
          .map{ |k,v| "'#{k}'" }

        #
        # Raise InvalidOptions if some invalid parameters were found
        #
        if unpermitted_params.length + required_params.length + wrong_types.length > 0
          ...
          raise ActsAsBookable::InvalidOptions.new(self, message)
        end
